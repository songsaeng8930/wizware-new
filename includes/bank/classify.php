...
{
    return [
        ['/통신|인터넷|KT|SKT|LG\s?U\+?|유플러스|텔레콤/iu', '525', 96], // 통신비
        ['/급여|월급|상여|봉급|임금|페이롤/u',                 '511', 95], // 급여
        ['/법인세|부가가치세|부가세|원천세|지방세|소득세|세금|과태료|공과금|국세|관세|전기료|수도료|가스료/u', '533', 97], // 세금과공과
        ['/임대|임차|월세|렌트|사무실\s?임/u',                 '521', 91], // 임차료
        ['/광고|마케팅|네이버\s?광고|구글\s?광고|google\s?ads|페이스북|인스타|퍼포먼스/iu', '523', 92], // 광고선전비
        ['/보험|화재보험|4대\s?보험|국민연금|건강보험|고용보험|산재/u', '532', 89], // 보험료
        ['/주유|주차|하이패스|택시|버스|지하철|KTX|항공|교통|출장/iu', '528', 86], // 여비교통비
        ['/식당|음식|커피|스타벅스|배달|회식|접대|유흥|골프/u',  '524', 70], // 접대비
        ['/편의점|GS25|CU|세븐일레븐|이마트|마트|복리|간식|경조/iu', '513', 78], // 복리후생비
        ['/쿠팡|문구|사무용품|소모품|오피스|토너|용지|비품/iu',  '526', 88], // 소모품비
        ['/수수료|용역|프리랜서|외주|컨설팅|대행|자문/u',        '522', 80], // 지급수수료
        ['/교육|강의|세미나|학원|연수|컨퍼런스|워크샵/u',        '529', 86], // 교육훈련비
        ['/차량|자동차|정비|타이어|엔진오일|세차/u',            '527', 84], // 차량유지비
        ['/이자|대출\s?이자|융자/u',                          '701', 90], // 이자비용
        ['/매입|원재료|자재|부품|상품\s?구매/u',               '501', 82], // 상품매입
    ];
}
...
{
    return [
        ['/이자|예금\s?이자/u',                              '601', 88], // 이자수익
        ['/임대료|월세\s?입금|렌트\s?수입/u',                 '404', 84], // 임대수입
        ['/용역|서비스|컨설팅|개발\s?대금/u',                 '403', 80], // 서비스매출
        ['/제품|완제품/u',                                   '402', 78], // 제품매출
        ['/상품|판매|매출|결제\s?대금|대금\s?입금|입금|수금/u', '401', 72], // 상품매출
        ['/환급|환불|잡수익|기타\s?수입/u',                   '602', 65], // 잡이익
    ];
}
...
function bank_classify_one(string $description, string $txType): array
{
    ...
    // 폴백 · 매칭 없음. 낮은 신뢰도로 수동확인 유도(입금=매출, 출금=소모품비 기본).
    $code = ($txType === '입금') ? '401' : '526';
