-- =============================================================
-- 급여 항목 동적 관리 마이그레이션
-- payroll_pay_types (마스터) + payslip_items (명세 항목)
-- =============================================================
...
CREATE TABLE IF NOT EXISTS payroll_pay_types (
    ...
    calc_base   ENUM('base_salary','gross_pay') NULL COMMENT 'rate 타입일 때 기준',
    calc_rate   DECIMAL(10,6) NULL COMMENT 'rate 타입일 때 요율',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='급여 지급/공제 항목 마스터';
...
-- 3. 기본 시드 (공제 항목) — 2026년 4대보험 요율 기준
--    국민연금: 9.5% 중 근로자 4.75% (국민연금법 시행령 2026.1.1 시행)
--    건강보험: 7.19% 중 근로자 3.595% (건강보험료율 고시 2026년)
--    장기요양: 건강보험료의 13.14%, 실효 근로자 부담 0.4724% (보건복지부 고시 2026년)
--    고용보험: 근로자 0.9% (고용보험법 시행령)
--    소득세: 간이세액표 기반이나 현 시스템은 간이 플랫 3% 적용
--    지방소득세: 소득세의 10%
INSERT INTO payroll_pay_types (code, name, category, is_taxable, calc_type, calc_base, calc_rate, has_hours, sort_order, is_system) VALUES
('NP',    '국민연금',     'deduct', 0, 'rate', 'base_salary', 0.047500, 0, 110, 1),
('HI',    '건강보험',     'deduct', 0, 'rate', 'base_salary', 0.035950, 0, 120, 1),
('LC',    '장기요양보험', 'deduct', 0, 'rate', 'base_salary', 0.004724, 0, 125, 1),
...
ON DUPLICATE KEY UPDATE name = VALUES(name), calc_rate = VALUES(calc_rate);
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='급여명세 항목별 금액';

-- 5. 기존 payslips 데이터 → payslip_items 이관
--    payslips 테이블에 데이터가 있고 payslip_items가 비어있을 때만 실행
