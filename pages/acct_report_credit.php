...
?>
        ...
        <div class="zm-tab-container mb-5">
            <a href="?tab=credit&subtab=sim"     class="px-5 py-2.5 text-sm font-medium border-b-2 transition-colors <?= $subTab==='sim'     ? 'approval-tab active' : 'approval-tab' ?>">고용증대 시뮬레이션</a>
            <a href="?tab=credit&subtab=history" class="px-5 py-2.5 text-sm font-medium border-b-2 transition-colors <?= $subTab==='history' ? 'approval-tab active' : 'approval-tab' ?>">시뮬레이션 이력</a>
        </div>
                        ...
                        <div>
                            ...
                            <div class="flex rounded-lg border border-slate-800 overflow-hidden">
                                <label class="flex-1 cursor-pointer">
                                    ...
                                    <span class="zm-tab zm-tab-active block text-center py-2 text-sm region-opt metro-opt">수도권</span>
                                </label>
                                <label class="flex-1 cursor-pointer">
                                    <input type="radio" name="region" value="province" class="sr-only" onchange="calcCredit()">
                                    <span class="zm-tab block text-center py-2 text-sm region-opt province-opt">비수도권</span>
                                </label>
                            </div>
                        </div>
                    </div>
        ...
        <div class="mb-4 flex items-center justify-between">
            ...
            <a href="?tab=credit&subtab=sim" class="inline-flex items-center gap-1.5 px-4 py-2 text-sm text-white bg-primary rounded-lg hover:opacity-90">
...
document.querySelectorAll('input[name="region"]').forEach(radio => {
    radio.addEventListener('change', () => {
        document.querySelectorAll('.region-opt').forEach(el => {
            el.classList.toggle('zm-tab-active', el.previousElementSibling && el.previousElementSibling.checked);
        });
        calcCredit();
    });
});
