...
function buildActions(d) {
    ...
    return `<div class="flex items-center justify-center gap-1">
        ...
        <div class="relative inline-block">
            <button onclick="event.stopPropagation();toggleActionMenu(this)" class="inline-flex items-center gap-1 px-1.5 py-1 text-xs rounded-md transition-colors" style="color:var(--zm-text-muted)" onmouseover="this.style.background='var(--zm-surface-hover)'" onmouseout="this.style.background=''"><i data-lucide="more-horizontal" class="w-4 h-4"></i></button>
            <div class="action-menu hidden absolute right-0 w-44 rounded-lg border shadow-xl z-50 py-1" style="background:var(--zm-surface-card);border-color:var(--zm-border)">
                <button onclick="event.stopPropagation();closeMenus();openChangeApprover(${d.id})" class="w-full text-left px-3 py-2 text-xs flex items-center gap-2 transition-colors" style="color:var(--zm-text-primary)" onmouseover="this.style.background='var(--zm-surface-hover)'" onmouseout="this.style.background=''"><i data-lucide="user-round-pen" class="w-3.5 h-3.5 text-primary"></i>결재자 변경</button>
